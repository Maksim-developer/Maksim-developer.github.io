window.onload = function () {
    ...
    const animateBlock = () => {
        ...
        const startTho = document.querySelector('.start-2');
        const startThree = document.querySelector('.start-3');
        const startFor = document.querySelector('.start-4');
        start.classList.add('animate-1');

        start.addEventListener('animationend', (e) => {
            start.classList.remove('animate-1');
            startFor.classList.remove('animate-1');
            startTho.classList.add('animate-1');
        });

        startTho.addEventListener('animationend', (e) => {
            start.classList.remove('animate-1');
            startTho.classList.remove('animate-1');
            startThree.classList.add('animate-1');
        });


        startThree.addEventListener('animationend', (e) => {
            startTho.classList.remove('animate-1');
            startThree.classList.remove('animate-1');
            startFor.classList.add('animate-1');
        });

        startFor.addEventListener('animationend', (e) => {
            startThree.classList.remove('animate-1');
            startFor.classList.remove('animate-1');
            start.classList.add('animate-1');
        });
    }
    const mainAnimate = document.querySelector('.header__logo');
    if (mainAnimate) {
        mainAnimate.addEventListener('animationend', (e) => {
            animateBlock();
        });
    }
    ...
    new ImageZoom(document.getElementById("tabs-images"), {
        ...
        "zoomWidth": 432,
        "scale": 0.1,
        "zoomStyle": 'width: 500px; height: 300px;',
        "zoomPosition": "top"
    });
    ...
    const fixedBlock = document.querySelector('.gim-booking'),
        ...
        andrometa = document.querySelector('.andrometa__slider'),
    ...
    const fixedScrollBlock = () => {
        ...
        if (scrollDistance > (buuttonOffsetTop) && scrollDistance <= (gim.offsetHeight + buuttonOffsetTop - andrometa.offsetHeight - fixedBlock.offsetHeight - header.offsetHeight)) {
    ...
    const spolilerTho = document.querySelector(".spoiler-2__box");
    const spolierCloseTho = document.querySelector(".spoiler-2__button");
    if (spolilerTho) {
        spolierCloseTho.addEventListener("click", function (e) {
            spolilerTho.classList.toggle("visible");
            spolierCloseTho.classList.toggle("active");
        })
    }
    const spolilerThree = document.querySelector(".spoiler-3__box");
    const spolierCloseThree = document.querySelector(".spoiler-3__button");
    if (spolilerThree) {
        spolierCloseThree.addEventListener("click", function (e) {
            spolilerThree.classList.toggle("visible");
            spolierCloseThree.classList.toggle("active");
        })
    }
